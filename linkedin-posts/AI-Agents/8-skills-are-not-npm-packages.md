...
His [opencode-skill-creator](https://github.com/antongulin/opencode-skill-creator) at ★123 proves the market wants this. And his background — Apple SDET, CooperVision Lead, Fortune 500 consultant — means he's not theorizing. He's shipping.

[IMAGE: Gulin's 3-Layer Architecture — Orchestration → Execution → Evidence, 6 quality gates. Screenshot from anton.qa]
...
[IMAGE: Example of a skill output where the model rates its own work as 9/10 — but the code doesn't compile. Confident garbage.]
...
**Layer 1 — Context:** AGENTS.md (boundaries, sources of truth) + Stop-rules (anti-patterns from failures)
**Layer 2 — Skill:** SKILL.md (domain knowledge, patterns) + Self-Evaluation (4 axes, no external judge)
**Layer 3 — Validation:** BDD Harness (executable Given-When-Then). Violation → build error, not stale doc
**Output:** Test code + feature files + behavior rules
...
**Self-evaluation axes:**

- **Correctness** — Does the output compile and pass? Measured by compiler, test run
- **Relevance** — Does it address the actual task? Measured by checklist match
- **Stability** — Same output given same input? Measured by 3-run variance
- **Coverage** — Does it handle edge cases? Measured by required patterns

[IMAGE: Self-evaluation table — 4 axes: correctness, relevance, stability, coverage]
...
[IMAGE: A CI build failing because a BDD spec detected code drift — the spec is red, the error message points to the exact violation]
...
[IMAGE: Comparison table — Gulin's code-output model (tool for testers, external judge, confident garbage) vs our behavior-output model (agent with guardrails, self-evaluation, stop-rule → learn → retry)]
