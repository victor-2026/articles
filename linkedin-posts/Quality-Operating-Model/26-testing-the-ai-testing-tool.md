...
- QA-IS-DEAD (JDAQA x Testkube, 17.09) — рыночное обоснование для 26: источник `ai-qa-wiki/raw/qa-is-dead-orchestrating-quality-2026.md`; вошла в цитатник quotes.md, секция QA is Dead. QA-доля инженерного коста **20% → 35% → 45%** (Traditional → AI SDLC → Agentic; grounded в Capgemini, Sonar 42% AI-assisted, LinearB 4.6X review wait) = validation становится primary cost → тезис "почему вообще тестировать тестировщика/вендора" (зачем 26-я существует). В тело 26 НЕ вставлять (заморожена), только в background/comments. Открытая платформа дока = Testkube (agentic testing) - см. notes 24/25.
