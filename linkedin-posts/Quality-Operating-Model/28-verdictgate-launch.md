...
Concrete case: our typical mock pattern **/api/posts* could never match /api/posts/feed — Twelve mocks in the existing suite were certifying nothing, flagged as debt. A planned probe proved it: the double-star glob intercepts the feed, the old pattern matches zero.
...
- ✅ PUBLISHED Mon Sep 22 (early, was scheduled Tue 09:00): Pulse `your-vendors-green-report-claim-heres-calculator-checks-victor-ematin-gml4f` + feed `urn:li:ugcPost:7507573088721567744` + first comment SENT. Day-0: 49 imp, 2 eng.
...
- ~~**T8 alt CTA (Perplexity action-variant, staged):** `Want the 30-second check for your own CSV? Full method + repo link in the article👇` + article URL (use instead of walkthrough bridge if tone needs more push).~~ ✅ APPLIED 21.09 as bridge B (adapted: "below", no URL per cover-click mechanics)
