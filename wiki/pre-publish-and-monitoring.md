...
- [ ] **Нет внешних URL в теле фид-поста** — LinkedIn режет охват. Ссылки в первый комментарий или через кнопку Pulse Article. В теле: «link in comments 👇». **Исключение — фид-пост Pulse Article:** он делается ПОСЛЕ обложки и статьи, в рамках той же публикации, автоматически подхватывает Cover, и в него ничего больше вставить нельзя. CTA в теле: «Full article below👇» (проверено на Article 22)
...
**Копирайт на последней странице (обязательно):** LinkedIn позволяет скачивать PDF карусели («For accessibility purposes... download your document as a PDF»). Последняя страница = подпись: `© Victor Ematin · AI Quality Engineering Lead · Independent practice`. Не подпись-афоризм, не ссылку на следующую статью - именно копирайт с именем и ролью.
